...
def validate(epoch, encoder, decoder, cross_entropy_loss, data_loader, word_dict, alpha_c, log_interval, writer):
    ...
    with torch.no_grad():
        ...
        with open('scalar_values.txt', 'a') as file:
             # Write the scalar values to the file
            file.write(f'Epoch: {epoch}\n')
            file.write(f'val_bleu1: {bleu_1}\n')
            file.write(f'val_bleu2: {bleu_2}\n')
            file.write(f'val_bleu3: {bleu_3}\n')
            file.write(f'val_bleu4: {bleu_4}\n')
            file.write(f'val_loss: {losses.avg}\n')
            file.write(f'val_top1_acc: {top1.avg}\n')
            file.write(f'val_top5_acc: {top5.avg}\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tf', action='store_true', default=True,
                        help='Use teacher forcing when training LSTM (default: True)')
